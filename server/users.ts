...
async function updateUsername(userId: number, username: string) {
  ...
  return { user: updatedUser, token: signToken(updatedUser) };
}
...
function createUpdateUsernameController(io: SocketIOServer) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      io.emit("online_users_updated", { users: await getOnlineUsers() });
      res.json(payload);
    } catch (error) {
    ...
    try {
      ...
      if (result.pendingRequest) {
        const receiverStatus = await getFriendshipStatus(
          result.pendingRequest.receiverId,
          result.pendingRequest.senderId,
        );
        const eventPayload = {
          requestId: result.pendingRequest.id,
          senderId: result.pendingRequest.senderId,
          receiverId: result.pendingRequest.receiverId,
          status: result.pendingRequest.status,
          senderStatus: result,
          receiverStatus,
        };

        io.to(`user:${result.pendingRequest.senderId}`).emit(
          "friend_request_updated",
          eventPayload,
        );
        io.to(`user:${result.pendingRequest.receiverId}`).emit(
          "friend_request_updated",
          eventPayload,
        );
      }
